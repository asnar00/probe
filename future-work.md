...
- **The rest of NEON and RVV**: `u1xN` in memory (a byte per lane: narrow and widen around `ld1`/`st1`), integer `min`/`max` as IR operations, conversions between lane widths that change signedness or skip a width (`u8x8` to `i32x8` is two vectors), and horizontals (`addv`, `faddp`, dot products) as library generics with rules.
