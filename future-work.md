...
- **Vectors whole elsewhere**: AIR takes them (`builtin vectors`) and arm64 takes the classed ones (NEON: `class v` and rules); riscv64 takes them on RVV (`ext V`) with every rule setting its own vtype — the simplest answer to a rule with machine state, one `vsetivli` per operation (a pass could elide the repeats). wasm SIMD would use the same seam — the parser keeps whole what a platform has a class and a rule for. The scalable model — `vl` as a value, loops in chunks of "as many as fit", masks for the tail — is untouched; `reference/scalable-vectors.md` is the design note; built (2026-08-29): slices `T[]` over buffers with a header, operations whole, `lib/slice.ssa` written over `chunk(T)` with `fit` — vector instructions on NEON and RVV, the elements one at a time elsewhere. Views of rank 1 to 3 followed (`suite/matrix.ssa`); then `dot`, `matmul` (a loop over rows and the transpose's rows — a platform's tile rule, SME or MFMA, would replace it), a row broadcast, and buffers from the heap; not there: rank above 3, a column broadcast without the transposes. What is not there: a chunk wider than the platform's constant (RVV's `vsetvli` on a wider VLEN would take more, and the type says 4), and a masked tail (`vl` for the last few elements; the tail runs one at a time).
- **Streams, the rest**: `lib/stream.ssa` has irregular streams — a ring, `push`, `frame`, `sample` (nearest, hold), `window` — with the design in `reference/streams.md`. `os/echo.ssa` and `os/clock.ssa` read `keys: u8$` and `tick: time$` from their handlers. Regular streams (`regular(s, dt, t0)`) and the linear rule are there. `sample img, x, y` on a spatial view is bilinear (`lib/sample.ssa`); a dense stream is pushed a block at a time and a node reads its history from the rings (`behind`, `last`; the biquad in `suite/stream.ssa`). Next, as one design (ash, 2026-08-29): **residency by analysis** — what every reader reads behind and ahead of its position, plus its lag under the schedule, is what a ring must hold, so capacity is derived, double and triple buffering are numbers (`x[t+dt] = f(x[t])` is 2, a biquad 3), a future read is a prefetch requirement, and a memory leak becomes a deadline miss (the `stale` check); **cost** — `probe cost` is there (SSA time, K, hardware time; `loop(...) bound N {`; `cost mnemonic = n` in a platform file, all ones until learned by measurement, which is next: an instruction class in a loop on the machine, cycles fitted, the learner's spirit); its first finding — a `push` into a regular stream cost ~300k SSA operations a sample in rational time arithmetic — led to rings keeping integer ticks of a clock with `time` at the boundary only (a push is ~500 now); **a virtual clock** — a `sim` platform for the JIT where `now`/`idle` are a deadline queue, so stream programs run as suite cases deterministically as fast as they can, with recorded inputs replayed from data; then the scheduler running a graph of stream nodes.
- **Frictions still there**: the statement form of a view operation (`add c, a, b`, no result) is the one place the syntax has two moods — by design, without an allocator, but a place to watch.
