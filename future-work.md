...
- **The rest of NEON and RVV**: 64-bit vectors (`i32x2`, `u8x8`: `.2s`/`.8b` in a d register), `u1xN` in memory (a byte per lane: narrow and widen around `ld1`/`st1`), integer `min`/`max` as IR operations, and horizontals (`addv`, `faddp`, dot products) as library generics with rules.
