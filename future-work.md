...
- **Vectors whole elsewhere**: AIR takes them (`builtin vectors`) and arm64 takes the classed ones (NEON: `class v` and rules); wasm SIMD and RVV would use the same seam — the parser keeps whole what a platform has a class and a rule for. RVV forces the question of a rule with machine state (`vsetvli`).
- **Vectors across a call**: a parameter, result or argument of a vector type is refused on arm64 ("pass its lanes"); the two-word convention is the natural one, and `fmov {x}, {v}.d[1]` / `fmov {v}.d[1], {x}` are learned for it.
- **The rest of NEON**: 64-bit vectors (`i32x2`, `u8x8`: `.2s`/`.8b` in a d register), `u1xN` in memory (a byte per lane: narrow and widen around `ld1`/`st1`), integer `min`/`max` as IR operations, and horizontals (`addv`, `faddp`, dot products) as library generics with rules.
