...
- **The rest of NEON and RVV**: integer `min`/`max` as IR operations, conversions between lane widths that change signedness or skip a width (`u8x8` to `i32x8` is two vectors), and horizontals (`addv`, `faddp`, dot products) as library generics with rules.
