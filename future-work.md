...
- **Simdgroup operations on 64-bit lanes**: Apple's intrinsics stop at 32 bits; a `simd_sum` on an `i64` is an error on the GPU. Two 32-bit halves with a carry, or a threadgroup reduction, would give it.
