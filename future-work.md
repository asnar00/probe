...
- **Streams, the rest**: `lib/stream.ssa` has irregular streams — a ring, `push`, `frame`, `sample` (nearest, hold), `window` — with the design in `reference/streams.md`. `os/echo.ssa` and `os/clock.ssa` read `keys: u8$` and `tick: time$` from their handlers. Regular streams (`regular(s, dt, t0)`) and the linear rule are there. Next: `sample img, u, v, bilinear` on a spatial view, the same operation; and the scheduler running a graph of stream nodes.
