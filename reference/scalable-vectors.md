...
The decision went a level higher than the note proposed: the program-facing type is a **slice**, `T[]`, a view into a buffer that carries its size (ash: "size should ride with the ptr"), and operations on slices are whole — `add c, a, b`, `s: f32 = sum a` — so `fit` never appears in a program. Underneath, `lib/slice.ssa` is written over **chunks**, `chunk(T)`: as many lanes of T as the platform's vector register holds, or T itself where it has none, with `lanes`, `sizeof` and `fit` as the platform's constants; a slice operation takes whole chunks while one is left and the elements one at a time after. So a `chunk(number)` body is NEON or RVV instructions on the machines and the scalar definition everywhere else — the same text, checked against itself. Option C's four questions were answered by construction: `fit` is an instruction (a `min`); the count does not ride with a register value at all but with the slice in memory; a slice crosses a call as its two words; and the chunk's width is the platform's, not `vl`'s. What remains of RVV proper is exactly that last point: a chunk is 128 bits because the platform says so, where a `vsetvli` on a 512-bit chip would say 16 lanes — the type would have to say "the machine's" for the same binary to widen, which is the scalable model this note described and the project has not taken.
