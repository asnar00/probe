...
- Newlines end instructions, and indentation is the structure (fm3 rulings-3, 2026-09-10): a block — a function's body, an `if` arm, a loop's body, a `struct` or `pack`'s fields, a `data` item's values, a platform's rules — is the lines indented under its header, and a line back at an earlier indentation ends every block opened since; a line must return to an indentation that is open. A flat function's basic block is its label with the instructions indented under it. Indent with spaces; a tab in the indentation is an error. The brace form the IR had before (`fn f() {` … `}`, `struct { … }`) is still read, with a warning naming the file's first braced line; the two may meet in one text (the prelude is appended to every program), and `probe indent <file>` rewrites a file in the indented form (`-w` in place).
...
```
data b: buffer(f32, 1024)                     ; a header (element size, capacity), then 1024 f32
p: ptr = addr b
a: f32[] = slice p                            ; the whole buffer: a view (data: ptr(f32), len: i64)
h: f32[] = view a, 512, 256                   ; a part of it, checked to lie within
n: i64 = len a
t: ptr(f32) = ptr a                           ; its data, for load/store by index
add c, a, b                                   ; c[i] = a[i] + b[i], into c — a statement, like store
mul c, a, 2.0                                 ; a scalar on the right
fill c, 0.0
copy c, a
...
d: i32 = dot r, c                             ; the sum of products of two rank-1 views
matmul p, a, b                                ; p[i, j] = dot of row i of a and column j of b, shapes checked
add c, a, r                                   ; a row broadcast: r added to every row (for a column, on the transposes)
b: ptr = buffer_take(h, 4, 1024)              ; a buffer from a heap (lib/heap.ssa); buffer_give(h, b) returns it
``` A **buffer** is memory with a header, the element size and the capacity a word each, then the elements: `data name: buffer(T, N)` declares one, `buffer_init(p, elem, cap)` (`lib/slice.ssa`) writes the header on memory carved at run time. A literal stored through a typed pointer or a view takes the element's type (`store 1.0, a, 0, 1`), and an index or count may be any integer, converted to `i64`. `slice p` takes the whole buffer and checks the element size against the view's — with counts, `slice p, n0, n1`, a view of that shape over it; `view a, off, n` a part of a rank-1 view, checked to lie within; `at a, i` the sub-view at i along the first axis, one rank down, checked; `transpose`, `block` and `reshape` (a contiguous rank-1 view seen with a shape, checked to be exactly it) as above; `len`, `stride`, `shape a, k` and `ptr` read the words. A view is a struct and travels as one — its words as arguments across a call, dissolved everywhere — and is otherwise a value like any other. A rank-2 view is sampled by coordinate as a texture is (`lib/sample.ssa`): `p: u8 = sample img, x, y` is bilinear, `x` along axis 1 and `y` along axis 0, a coordinate an index (`(0, 0)` the first element exactly), the edges clamped; the weights are the coordinate's type — any `scalar` — into which each element is converted and out of which the result is rounded once, so a byte image sampled at `f32` coordinates gives a byte. The same word samples a stream by time (*Streams*). A literal after a view or a stream in an operation form takes the parameter's type rather than the first operand's — `peek s, 2` an `i64`, and where the parameter is abstract, its own kind's, `sample img, 1.5, 0.5` two `f64`s.

An operation on slices is written like a store, with no result, and writes into its first operand: `add c, a, b`, `sub`, `mul`, `div`, `min`, `max` with two slices or a slice and a scalar, `neg`, `abs`, `sqrt` (on `scalar[]`) with one, `copy c, a`, `fill c, k`; lengths must agree, which is a `check`. A reduction — `sum`, `min`, `max` — gives a scalar. Each is a generic of `lib/slice.ssa` over `number[]` (the tower binds `number` to the element type); an operation of rank 2 or 3 is the rank below it over each row (`at`), a rank-1 view on the right of a rank-2 operation is broadcast to every row, a reduction one rank down — `sum c, a` with `c` a rank-1 view and `a` rank 2 — is one per row (for the columns', reduce the transpose), `dot` is a reduction of a product, and `matmul` is `dot` over rows and the transpose's rows — a loop until a platform has a rule for a tile. The rank-1 body is the definition — and the machine's form at once, through **chunks**:
...
```
data vb: buffer(u8, 8)                        ; an irregular ring: values and a tick per item, twice the capacity kept
data tb: buffer(i64, 8)
data rg: array(u64, 8)                        ; the header: pushed, base, the buffers, the clock (hz, step, t0)
ring_init(r, a, b, 1000000)                   ; a ring on a microsecond clock
s: u8$ = stream r                             ; a reader's view: position 0, nearest, failing at the edges
push s, 1000, 9                               ; the producer's side: (tick, v), ticks non-decreasing (checked)
f: u8[], k0: i64, s2: u8$ = frame(s)          ; everything since this reader looked, the index of its first item, the reader moved on
t: time = time_at(s, k0)                      ; an item's time: its tick over the rate, exact
n: i64 = count s                              ; how much is waiting
x: u8 = sample s, t                           ; the value at a time, by the view's rule: nearest, hold (1), linear (2)
h: u8$ = sampling(s, 1)                       ; another view of the same ring, holding
w: u8[] = window s, t1, t2                    ; the items with t1 <= t < t2
v: u8 = latest s
x: u8 = peek s, 2                             ; the third unread item, the reader not moved
s2: u8$ = advance(s, 2)                       ; the reader moved on two
ring_regular(r, a, 48000, 1, 0)               ; a regular ring: item k at tick t0 + k * step, no ticks stored
push reg, 10.0                                ; its producer's push: no tick
push reg, block                               ; a block at a time
hx: f32[] = behind reg, 2                     ; the two items before this reader's position
hy: f32[] = last out, 2                       ; the two last pushed: a producer's own history
```

`T$` is a **stream** of T: a value over time, seen as an array indexed by `time` — `x[t]` — with a rule for the times between samples, as a texture has for the points between texels. It is a *reader's view* of a **ring**: the ring is the producer's — a header with how many items have been pushed and where the resident ones start, the values buffer, and the ring's **clock** — and the view carries the reader's position, a sampling rule and an edge rule, so several streams may look at one ring, each with its own rules, as several slices look at one buffer. A stream is a struct of four words and travels as one; a reader's position threads through its loop as an SSA value, which is a program over streams written as SSA. Time inside a ring is **integer ticks** of its clock — what the machine's counter and a device's sample index are: an irregular ring (`ring_init(r, vb, tb, hz)`) keeps a tick per item, and a regular ring (`ring_regular(r, vb, hz, step, t0)`) keeps none, item k being at `t0 + k·step`. `time` — exact seconds — is the boundary type: `time_at(s, k)` divides a tick by the rate, `sample s, t` and `window s, t1, t2` multiply a time by it once and do the rest in integers, and two rings on different clocks meet by one such conversion where they meet; so a producer's push and a reader's frame do integer work only (`probe cost`: a push into a regular ring is 30 SSA operations — its two checks, the slot from the buffer's own header, two stores through a pointer to the elements since the slot is under half by construction, the count and the base — and `peek` and `latest` read one item back through the same pointer at 18, `frame` and `unread` packing their view from it at 17; with a view packed per read it was 34, with a slide when the ring filled a push was 222, and with a rational time per item 300 000). The resident items stay contiguous and nothing ever slides — the buffers are twice the resident count and each item is stored in both halves, item k at slot `k mod half` and again half further on, so the last half items are one view wherever the seam falls (`suite/stream.ssa`'s `wrapped`) — so a `frame` or a `window` is a view (chunks apply), and a reader that fell further behind than the resident items fails a `check`, as does a sample before them all unless the view's edge rule clamps. Every operation is `lib/stream.ssa`'s, over `any$`, an abstract name above `number` in the tower whose family is every type — so **a ring may hold a struct**, one buffer whose stride is the struct's size, one header, one position, and a struct's push is one push (`suite/stream.ssa`'s `struct_ring` and `struct_frame`); the two words that weigh two items and round between them, `sample`'s linear rule and `lerp`, stay over `number` and refuse a struct ring by name, there being no midpoint between two of them. A reader that wants less than a frame — a line reader stopping at its newline — `peek`s at the unread items and `advance`s past what it took. On the machines the producers are the interrupt handlers: `os/echo.ssa`'s `__irq` pushes each byte the port has into `keys: u8$` at the counter's tick, and `read` — a reader kept across system calls in `data` — sleeps until a newline is among the unread bytes and moves on past it; `os/clock.ssa`'s pushes each tick's count into `tick: i64$` and the program reads it a frame at a time, the tenth's count becoming milliseconds at the end. The rules are nearest (0), hold (1) and linear (2): linear weighs the two items around `t` in exact rational arithmetic and rounds once into the element's type — `26.0` between `20.0` at 1 ms and `30.0` at 2 ms, `2` between bytes 2 and 3 at 2.75 ms. On a spatial view the same word is bilinear (*Slices and buffers*). A dense regular stream is pushed a block at a time (`push s, block`, a view in, copied whole from its slot with the IR's `copy` — the slot under half and the block at most half, it never reaches the end, and past the seam it lands on the twin positions — and again half further on, split where that reaches the end, three copies and the count moving once; `push s, tick, block` on an irregular ring, every item at that tick; a block longer than half the slots fails, since it could never be resident whole), `received s` is how many items the ring has had from its producer, and a node reads its history from the rings rather than keeping state of its own: `behind s, k` is the k items before this reader's position, `last s, k` the k last pushed — fewer at a stream's start, `tail(h, k)` giving 0 before the beginning — so `suite/stream.ssa`'s biquad reads two behind its frame of the input and two behind its own output, and its decay carries across blocks through the output ring. What a node reads behind and ahead of its position is what a ring must keep resident, which is the number a residency analysis should compute in place of the declared capacity; with a bound on each node's time, a scheduler running a graph of stream nodes — both are next.
