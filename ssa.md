...
reg: f32$ = regular(s, dt, t0)                ; the ring seen as regular: a sample every dt from t0
push reg, 10.0                                ; its producer's push: the time is computed
l: f32$ = sampling(reg, 2)                    ; linear: the two items around t weighed, exactly
