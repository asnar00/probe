# countdown
*extends hello: a countdown from 10 at one hertz before the greeting, static on*
...
- Static on (log 71): `run` is bye's body calling `run__countdown`, this feature's, by name, with no gate; there is no `with countdown off` context.
